...
export default function LoveScreen() {
  ...
  const [loveVerses, setLoveVerses] = useState<Verse[]>([]); // State to hold all verses with the "Love" theme

  useEffect(() => {
    // Filter verses with the theme "Love"
    const filteredVerses = bibleVerses.filter((verse: Verse) => verse.theme === "Love");
    setLoveVerses(filteredVerses);
  ...
  const shuffleVerse = () => {
    if (loveVerses.length > 0) {
      const randomIndex = Math.floor(Math.random() * loveVerses.length);
      setCurrentVerse(loveVerses[randomIndex]);
    }
  };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.mainSection} pointerEvents="box-none">
          {currentVerse && (
            <>
              <Text style={styles.themetitle}>Theme: Love</Text>
