...
export default function ResilienceScreen() {
  ...
  const [resilienceVerses, setResilienceVerses] = useState<Verse[]>([]); // State to hold all verses with the "Resilience" theme

  useEffect(() => {
    // Filter verses with the theme "Resilience"
    const filteredVerses = bibleVerses.filter((verse: Verse) => verse.theme === "Resilience");
    setResilienceVerses(filteredVerses);
  ...
  const shuffleVerse = () => {
    if (resilienceVerses.length > 0) {
      const randomIndex = Math.floor(Math.random() * resilienceVerses.length);
      setCurrentVerse(resilienceVerses[randomIndex]);
    }
  };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.mainSection} pointerEvents="box-none">
          {currentVerse && (
            <>
              <Text style={styles.themetitle}>Theme: Resilience</Text>
